...
<div x-data="{ type: 'password' }">
    ...
    <div class="relative">
        <input :type="type"
            ...
            name="{{ $name ?? 'password' }}" placeholder="{{ $placeholder ?? '******' }}" value="{{ old($name ?? '') }}" />
        <p class="absolute right-12 bottom-0 h-full flex items-center" x-show="type === 'text'" @click="type = 'password'">
