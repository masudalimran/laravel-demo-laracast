@props(['label', 'name', 'maxChar', 'prevData'])
@php
    $dummyData =
...
@endphp
<div x-data="{ content: '{{ isset($prevData) ? $prevData : old($name ?? '') }}', hasError: true }" class="my-4">
    <label for="{{ $name ?? 'body' }}" class="w-full font-semibold">
        {{ $label ?? '' }}
    </label>
