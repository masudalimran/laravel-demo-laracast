...
@endphp
<div x-data="{ content: '{{ old($name ?? '') }}', hasError: true }" class="my-4">
    <label for="{{ $name ?? 'body' }}" class="w-full font-semibold">{{ $label ?? '' }}</label>
    <textarea rows="{{ isset($maxChar) ? $maxChar / 200 : 15 }}" name="{{ $name ?? 'name' }}"
    ...
    <div class="flex justify-between items-center my-2 gap-4">
        <button class="px-2 py-1 text-xs bg-primary hover:opacity-70 rounded text-white" type="button"
            x-on:click="content = '{{ $dummyData }}'">Generate Dummy
            Data</button>
        <p class="font-extralight mr-2 "><span x-text="content.length"></span>/{{ $maxChar ?? 3000 }}</p>
    </div>
    @error($name)
